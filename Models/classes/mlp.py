...
class MLP(Model):
    """
    Multi Layer Perceptron. Especializada para
    múltiples etiquetas.

    Model: Clase padre de todos los modelos implementados.
    """
    MODEL = "MLP"
    with open('Models/parameters/models_schemas/mlp.json') as f:
        schema = json.load(f)
    
    def __init__(self, **kwargs):
        """
        Inicializa una instancia de MLP.
    ...
    def get_model(self, n_inputs, n_outputs, depth, activation, optimizer):
        ...
        """
        model = Sequential()
        model.add(Dense(depth, input_dim=n_inputs, kernel_initializer='he_uniform'))
    ...
    def fit(self, x, y):
        ...
        # get model
        self.mlp = self.get_model(n_inputs,
                                    n_outputs,
                                    self.params['depth'],
                                    self.params['activation'],
                                    self.params['optimizer'])
        # fit the model on all data
        self.mlp.fit(x,y,verbose=self.params['verb'],epochs=self.params['epochs'])
    ...
    def predict_proba(self, x):
        ...
        if x.ndim == 1:
            x = x.reshape(1, -1)
        return pd.DataFrame(self.mlp.predict_proba(x)).astype(np.float64)
    ...
    def save(self, filename=None):
        ...
        weigth_bytes = BytesConverter(self.mlp.get_weights()) 
